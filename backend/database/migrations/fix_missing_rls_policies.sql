-- Fix missing RLS policies identified by security enforcement CI
-- =========================================
...
-- Users can see their own profile within their tenant
...
-- Admins can insert new tenants (system initialization)
...
with check (
  ...
  and public.is_admin(auth.uid(), id)
);
...
-- Admins can update audit logs (corrections)
...
-- Fix tenants_admin_update to use tx_tenant_id pattern
...
-- Fix tenants_admin_delete to use tx_tenant_id pattern
...
-- Ensure kv_store is completely blocked
