...
class SimpleRequestTracer {
  ...
  middleware() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      res.end = function(this: Response, ...args: any[]) {
        ...
        // Store trace
        SimpleRequestTracer.traces.push(trace);
        
        // Keep only recent traces
        if (SimpleRequestTracer.traces.length > SimpleRequestTracer.maxTraces) {
          SimpleRequestTracer.traces = SimpleRequestTracer.traces.slice(-SimpleRequestTracer.maxTraces);
        }

        // Log slow requests
        if (responseTime > SimpleRequestTracer.slowThreshold) {
  ...
  static getMetrics() {
    const recentTraces = SimpleRequestTracer.traces.slice(-100);
    ...
    if (recentTraces.length === 0) {
      return {
        ...
        requestsPerMinute: 0,
        totalRequests: 0,
        slowThreshold: SimpleRequestTracer.slowThreshold
      };
    }
    ...
    const slowestRequests = recentTraces
      .filter(t => t.responseTime > SimpleRequestTracer.slowThreshold)
      .sort((a, b) => b.responseTime - a.responseTime)
      .slice(0, 10);
    
    const errorRate = recentTraces.filter(t => t.statusCode >= 400).length / recentTraces.length * 100;
    const requestsPerMinute = SimpleRequestTracer.calculateRequestsPerMinute(recentTraces);
  ...
  static getRouteMetrics() {
    ...
    SimpleRequestTracer.traces.forEach(trace => {
...
export const simpleRequestTracer = new SimpleRequestTracer();
